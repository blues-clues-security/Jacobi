...
# Start Main Menu
	'title': "Main Menu", 'type': MENU, 'subtitle': "Jacobi Viewer for Moloch\n    Below you will see predetermined queries for network analysis \n    Please select a query and give the appropriate input to open a Google Chrome session with the selected view",
	'options':[
...
# Start Query Menu
		{ 'title': "Select Moloch Queries", 'type': MENU, 'subtitle': "Please select a query...",
		'options': [
...
#		]
#		},
			{ 'title': "Inbound Connections", 'type': MENU, 'subtitle': "For this query you will see all incoming connections with the count of times they are in ip.src\n    *NOTE* These connections are regardless of where they came from, or where they are going to\n    You can leave time blank to see the last 6 hours",
			'options': [
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
				{ 'title': "Input End Time", 'type': PYTHON, 'python': 'endtimeinput()' },
				{ 'title': "Clear time for this session", 'type': PYTHON, 'python': 'clear_time()' },
			...
			{ 'title': "Outbound Connections", 'type': MENU, 'subtitle': "For this query you will see all incoming connections with the count of times they are in ip.dst\n    *NOTE* These connections are regardless of where they came from, or where they are from\n    You can leave time blank to see the last 6 hours",
			'options': [
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
				{ 'title': "Input End Time", 'type': PYTHON, 'python': 'endtimeinput()' },
				{ 'title': "Clear time for this session", 'type': PYTHON, 'python': 'clear_time()' },
			...
			{ 'title': "Key Terrain -> Key Terrain Connections", 'type': MENU, 'subtitle': "For this query you will see all connections from one list and to an IP in the same list",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
			...
			{ 'title': "Key Terrain -> Defended Asset Connections", 'type': MENU, 'subtitle': "For this query you will see all connections from one list and to IPs in the list entered in 'Open View' \n    You\'re inputting the Destination IP's when you enter \'Open View\' \n    You still need to set the source IPs in global variables, or in options 1 or 2 on this screen!",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
			...
			{ 'title': "DAL -> DAL Connections", 'type': MENU, 'subtitle': "For this query you will see all connections from one list and to an IP in the same list",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
			...
			{ 'title': "Server -> Server Traffic", 'type': MENU, 'subtitle': "For this query you will see all connections from one list and to an IP in the same list",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
			...
			{ 'title': "Workstation -> Workstation Connections", 'type': MENU, 'subtitle': "For this query you will see all connections from one list and to an IP in the same list",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
...
#					]
#					},
			{ 'title': "Find Uncommon Websites Searched", 'type': MENU, 'subtitle': "For this query you can use a start and end time, or use the last 6 hours if no time is entered\n    This will search for any http connections attempted to domains not commonly found\n    Reference line number 807 in the code for domain list or to update",
			'options': [
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
				{ 'title': "Input End Time", 'type': PYTHON, 'python': 'endtimeinput()' },
...
#				]
#				},
			{ 'title': "Non-military Connections (Unidentified IPs)", 'type': MENU, 'subtitle': "Please select an option...\n    This query takes 2 IP list\n    Use option 2 or set in the GLOBAL menu to set the SOURCE IPs and use option 5 to set the DESTINATION IPs",
			'options': [
				{ 'title': "Input IP Address", 'type': PYTHON, 'python': 'iplist()' },
				{ 'title': "Input targets file", 'type': PYTHON, 'python': 'ipfile()' },			
				{ 'title': "Input Start Time", 'type': PYTHON, 'python': 'starttimeinput()' },
...
#				]
#				},
# I started this function below with port_mismatch() but not sure how to get Moloch to show this information
...
#				]
#				},
# I know how I would query this, but need an example to make sure that it would work. Something like (protocols ==http ) || (protocols == dns)
...
#					]
#					},
# Another one that I would need an example to figure out how to filter this 
...
# End Query Menu
# Start Help Menu
		{ 'title': "Help", 'type': MENU, 'subtitle': "Please select a topic to get more information...\n    You'll notice this is similar to the main menu\n    Choosing a help topic will display information in the terminal for 10 seconds",
		'options': [
			{ 'title': "Help for 'Define Global Variables (date/time/IP)'", 'type': MENU, 'subtitle': "Please select an option...\n    *NOTE*\n    These options will be set GLOBALLY across the program, so you don't need to re enter information with each query",
			'options': [
				{ 'title': "Input IP Address", 'type': COMMAND, 'command': 'echo "This is where you will input an IP list into the command prompt separated by spaces";echo "";echo "You probably won\'t use this much, except to enter 1 or 2 IPs at a time";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				{ 'title': "Input targets file", 'type': COMMAND, 'command': 'echo "This is where you\'ll enter a text file to be used as an IP list";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				{ 'title': "Input Start Time", 'type': COMMAND, 'command': 'echo "This will take you through a series of inputs to enter the Year, Month, Day, Hour, and Minute to begin monitoring";echo "";echo "Once this is set once you don\'t need to set it again";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				{ 'title': "Input End Time", 'type': COMMAND, 'command': 'echo "This will take you through a series of inputs to enter the Year, Month, Day, Hour, and Minute to end monitoring";echo "";echo "Once this is set once you don\'t need to set it again";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				{ 'title': "Clear IPs for this session", 'type': COMMAND, 'command': 'echo "This will clear any IPs you have entered either from file or in a list for this session";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				{ 'title': "Clear time for this session", 'type': COMMAND, 'command': 'echo "This will clear any times you have entered for this session";echo "";echo "Press ctrl + C to quit out of this help page early";sleep 10' },
				]
				},
		]
		},
...
moloch_default = 'http://localhost:8005'
...
# Start port/protocol mismatch query
def port_mismatch():
	try: browser
	except NameError:
		try: moloch
	...
	while x !=ord('\n'):
      ...
      # Display all the menu items, showing the 'pos' item highlighted
			for index in range(optioncount):
				...
				if pos==index:
					textstyle = h
				try: screen.addstr(8+index,4, "%d - %s" % (index+1, menu['options'][index]['title']), textstyle)
				except Exception:
					curses.endwin()
					os.system('clear')
      ...
      # Now display Exit/Return at bottom of menu
			...
			if pos==optioncount:
				textstyle = h
			screen.addstr(9+optioncount,4, "%d - %s" % (optioncount+1, lastoption), textstyle)
			screen.refresh()
