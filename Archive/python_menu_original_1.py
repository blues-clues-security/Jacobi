...
menu_data = {
  'title': "Jacobi Viewer for Moloch", 'type': MENU, 'subtitle': "Please select an option... \n Another line \n One mo line",
  'options':[
  ...
  while x !=ord('\n'):
    if pos != oldpos:
      ...
      for index in range(optioncount):
        ...
        if pos==index:
          textstyle = h
        screen.addstr(10+index,4, "%d - %s" % (index+1, menu['options'][index]['title']), textstyle)
...
def processmenu(menu, parent=None):
  ...
  while not exitmenu: #Loop until the user exits the menu
    ...
    if getin == optioncount:
      ...
      os.system('reset')
      #if menu['options'][getin]['title'] == 'Pianobar':  # Leaving as an example of calling commands differently
