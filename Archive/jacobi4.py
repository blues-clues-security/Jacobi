...
# The need to find highest amount of traffic and least amount was accomplished in the same view
...
def starttimeinput():
  ...
  startdatetime_input = (int(inputstartyear),int(inputstartmonth),int(inputstartdate),int(inputstarthour),int(inputstartmin)) #Cast each input as an int to properly pass to datetime.datetime
  timeoffset = datetime.datetime(int(inputstartyear),int(inputstartmonth),int(inputstartdate),int(inputstarthour),int(inputstartmin)) - datetime.timedelta(seconds = time.timezone) #Set startdatetime_input to seconds to calculate offset
  starttime = ((timeoffset + datetime.timedelta(hours=5)) - datetime.datetime(1969,12,31,18)).total_seconds() #Calculate offset based off Moloch's epoch start time which is 12/31/1969 18:00 and add 5 hours to timeoffset to calculate difference between UTC and Central
...
def outbound_connections():
  ...
  try: starttime,endtime
    ...
    os.system ("nohup google-chrome '192.168.108.136:8005/unique.txt?counts=1&exp=ip.src&date=6' 2>/dev/null &")
    os.system ('sleep 5')
  else:
    timestamp = str('&stopTime='+str(int(endtime))+'&startTime='+str(int(starttime)))
    os.system ("nohup google-chrome '192.168.108.136:8005/unique.txt?counts=1&exp=ip.src" + timestamp + "' 2>/dev/null &")
...
def inbound_connections():
  ...
  try: starttime,endtime
    ...
    os.system ("nohup google-chrome '192.168.108.136:8005/unique.txt?counts=1&exp=ip.dst&date=6' 2>/dev/null &")
    os.system ('sleep 5')
  else:
    timestamp = str('&stopTime='+str(int(endtime))+'&startTime='+str(int(starttime)))
    os.system ("nohup google-chrome '192.168.108.136:8005/unique.txt?counts=1&exp=ip.dst" + timestamp + "' 2>/dev/null &")
# End inbound connections query function
"""
# Start KTC to KTC connections function
#
#def ktc2ktc_connections():
#	try: ip_range
#	except NameError:
#	
#	else:		
...
#
#else:
#		if ip_range exists or ktc2_iplist = ip_target_file
#		except NameError:
#			print ('You have not setup a global iplist')
#	    os.system ('sleep 5')
#		else:
# 	   i = 0
# 	   uri_ip2 = ''
...
#		 os.system ("nohup google-chrome '192.168.108.136:8005/unique.txt?counts=1&exp=ip.dst")
"""
"""
